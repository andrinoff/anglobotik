...
async def start(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(
        "Привет! Я Англоботик, который поможет проверить твою письменную работу для экзаменов. Выбери, какой экзамен ты сдаёшь: ОГЭ или ЕГЭ:",
...
async def EgeOrOge(update: Update, context: CallbackContext):
    ...
    if query.data == "EGE":
        ...
        await query.edit_message_text(
            text="Отлично! Теперь выбери, что будем проверять: эссе или личное письмо.",
            reply_markup=reply_markup,
        )
    ...
    if query.data == "OGE":
        ...
        reply_markup = telegram.InlineKeyboardMarkup(keyboard)
        await query.edit_message_text(
            text="Отлично! Теперь выбери, что будем проверять",
            reply_markup=reply_markup,
        )
        return LETTER_OR_ESSAY
...
async def letterOrEssay(update: Update, context: CallbackContext):
    ...
    if query.data == "LETTER":
        ...
        reply_markup = telegram.InlineKeyboardMarkup(keyboard)
        await query.edit_message_text(
            text="Привет! Я Англоботик, который поможет проверить твою письменную работу для экзаменов. Выбери, какой экзамен ты сдаёшь: ОГЭ или ЕГЭ:",
            reply_markup=reply_markup,
        )
        return EGE_OR_OGE
    else:
        await query.edit_message_text(
            text="Error: Unexpected button pressed. Please try again."
        )
        return ConversationHandler.END
    keyboard = [
        [
            telegram.InlineKeyboardButton(
                f'🔹 Учитель ({os.getenv("PRICE")}, детальный разбор)',
                callback_data="TEACHER",
            )
        ],
        [
            telegram.InlineKeyboardButton(
                "🔹 Искусственный интеллект (бесплатно, быстрая проверка)",
                callback_data="AI",
            )
        ],
        [telegram.InlineKeyboardButton("Назад", callback_data="BACK")],
    ...
    reply_markup = telegram.InlineKeyboardMarkup(keyboard)
    await query.edit_message_text(
        text="Последний шаг! Кто будет проверять твою работу? \nВыбери вариант:",
        reply_markup=reply_markup,
    )
...
async def TeacherOrAi(update: Update, context: CallbackContext):
    ...
    if query.data == "TEACHER":
        ...
        print("they have pressed back")
        if context.user_data["type_exam"] == "EGE":
            # Adds the EGE to the python-telegram-bot memory about users
            # and asks for possible exams
            print("they have ege selected")
            keyboard = [
                [telegram.InlineKeyboardButton(f"Эссе", callback_data="ESSAY")],
                [
                    telegram.InlineKeyboardButton(
                        "Личное письмо", callback_data="LETTER"
                    )
                ],
                [telegram.InlineKeyboardButton("Назад", callback_data="BACK")],
            ]
        ...
        if context.user_data["type_exam"] == "OGE":
            ...
            print("they have ege selected")
            keyboard = [
                [telegram.InlineKeyboardButton("Эссе", callback_data="ESSAY")],
                [
                    telegram.InlineKeyboardButton(
                        "Личное письмо", callback_data="LETTER"
                    ),
...
async def teacher(update: Update, context: CallbackContext):
    ...
    await context.bot.send_message(
        chat_id=os.getenv("TEACHER_CHAT_ID"),
        text=f"@{update.effective_chat.username} sent you a {context.user_data['type'].lower()} to check {context.user_data['type_exam']}: \n {work}",
    )
    await update.message.reply_text(
        f'Отлично. Твой текст был отправлен учителю. Оплати по {os.getenv("PHONE_NUMBER")} и отправь чек в личку! \n Их контакты: {os.getenv("NAME")}, @{os.getenv("USERNAME")}'
    )
    return ConversationHandler.END
